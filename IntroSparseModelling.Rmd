...
mu<-rep(0, 1000)
Sigma<-diag(1000)
x<-matrix(mvrnorm(n=1000, mu=mu, Sigma=Sigma),nrow=length(mu))
Y<- 1.1*x[,1] +0.7*x[,2]+ 0.3*x[,3]+0.15*x[,4]+0.05*x[,5] + rnorm(1000, 0, 0.5)
...
```{r glmnet}
 ...
 elastic.cv <- cv.glmnet(x,Y,type.measure = "deviance", nfolds = 5, alpha = 0.5)
  cor.test(matrix(coef(elastic.cv)[2:1001, ]), sim_slopes) 
  
  ridge.cv <- cv.glmnet(x,Y,type.measure = "deviance", nfolds = 5, alpha = 0)
  cor.test(matrix(coef(ridge.cv)[2:1001, ]), sim_slopes)
...
ERYN: STILL NEED TO ADD PREDICTION TO SUSIE!!!
...
In groups, analyze the Unicorn data with Susie, Lasso, Ridge and ElasticNet. Which genetic architectures can be recovered with these different methods?
...
In groups, analyze the Unicorn data with Susie, Lasso, Ridge and ElasticNet. Which method can best predict where the dragons are going to be?
