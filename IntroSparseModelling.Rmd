...
The point of this intro is to follow up from the powerpoint deck that I used on 9th May 2022 at SEED. I also recommend these two videos as a very basic jumping off point:
...
Sparse Modelling is a general framework to 1) choose among predictor values when n less than p, 2) increase the interpretablity of models (using fewer predictors) and 3) prevent overfitting. 
...
3) elastic net
4) Susie
I also briefly mentioned GEMMA's BSLMM.
...
mu<-rep(0, 100)
Sigma<-diag(100)
x<-matrix(mvrnorm(n=100, mu=mu, Sigma=Sigma),nrow=length(mu))
Y<- 1.1*x[,1] +0.7*x[,2]+ 0.3*x[,3]+0.15*x[,4]+0.05*x[,5] + rnorm(100, 0, 0.5)

## for later, this means our real slopes should be:
sim_slopes<-c(1.1, 0.7, 0.3, 0.15, 0.05, rep(0, 95))
```
Excellent, now we have some data, I'm gonna run through ridge, lasso and elastic net in glmnet. Ridge and lasso can both be done in monomvn as well, but as far as I can tell, elasticnet can't bet, so ymmv.
...
```{r glmnet}
 ...
 elastic.cv <- cv.glmnet(x,Y,type.measure = "deviance", nfolds = 5, alpha = 0.5)
  cor.test(matrix(coef(elastic.cv)[2:101, ]), sim_slopes) 
  
  ridge.cv <- cv.glmnet(x,Y,type.measure = "deviance", nfolds = 5, alpha = 0)
  cor.test(matrix(coef(ridge.cv)[2:101, ]), sim_slopes)
     
lasso.cv <- cv.glmnet(x,Y,type.measure = "deviance", nfolds = 5, alpha = 1)
  cor.test(matrix(coef(lasso.cv)[2:101, ]), sim_slopes)

```

glmnet has a bunch of internal functions (like coef above, note I used matrix(coef) because otherwise it prints . instead of 0 because sparse), print and predict. Predict is of course useful for when we have predictor variables, but no independent variables. 
```{r CV plots}
predict(elastic.cv, newx = matrix(mvrnorm(n=100, mu=rep(0,5), Sigma=diag(5)),nrow=5))
```


###Susie

Okay, the next method we can look at is Susie. Remember that Susie gives sets of variables, so isn't as bothered if variables are correlated. 
L is Maximum number of non-zero effects in the susie regression model. If L is larger than the number of covariates, p, L is set to p. Basically, how many parameters do you think are important (even if you don't know which are which).

```{r Susie}
   susie <- susie(x,Y,L = 10, max_iter = 1000)
    cor.test(coef(susie)[-1],sim_slopes)
    hist(susie$pip) ### these are the posterior inclusion probabilities, aka is this parameter included as an important parameter in the sparse framework?
```